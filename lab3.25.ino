...
void loop(void)
{
  ...
  Serial.println("\r\nPlace your NDEF formatted NTAG213 tag on the reader to update the");
  ...
  if (success)
  {
    ...
    if (uidLength != 7)
    {
      Serial.println("This doesn't seem to be an NTAG213 tag (UUID length != 7 bytes)!");
    }
    else
    {
      uint8_t data[32];

      Serial.println("Seems to be an NTAG213 tag (7 byte UID)");
      memset(data, 0, 4);
      ...
      else
      {
        ...
        else
        {
          ...
          // Try to add a new NDEF URI record
