...
const char *outTopic = "yimingt/data";
const char *inTopic = "yimingt/led";
...
void mqttReconnect() {
    ...
    while (!mqttClient.connected()) {
        ...
        // Attempt to connect
        if (mqttClient.connect("esp32Client_yimingt")) {
            Serial.println("connected");
            mqttClient.publish(outTopic, "Hello! This is yimingt's ESP32!");
...
void loop()
{
    ...
    if (millis() - lastMqttPublish >= mqttPublishInterval) {
        if (mqttClient.connected()) {
            mqttClient.publish(outTopic, "yimingt's heartbeat");
        }
